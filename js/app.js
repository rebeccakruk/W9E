...
// the while loop results in tweets, usernames, and created-at dates published by people 18 years of age or older
counter = 0;
while (counter < twits.length) {
    if (twits[counter].age >= 18) {
        console.log(twits[counter].tweet);
        console.log(twits[counter].userName);
        console.log(twits[counter].createdAt);
        counter = counter + 1;
    }
...
// the for loop results in tweets from those under 18 years of age
// the for loop covers all the aspects of a loop in one line
for (let i = 0; i < twits.length; i++) {
    if (twits[i].age < 18) {
        console.log(twits[i].tweet);
        console.log(twits[i].userName);
        console.log(twits[i].createdAt);
    }
}